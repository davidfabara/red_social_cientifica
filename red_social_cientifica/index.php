...
if(isset($_POST['publicar']) and !empty($_FILES) and !empty($_POST['titulo']))
{
    ...
    */
	...
	$refRes1=$_POST['refResumen1'];$refRes2=$_POST['refResumen2'];
	$referenciasResumen=(!empty($refRes1) and !empty($refRes2))?"."."$refRes1"."( ".$refRes2. " )"."<hr>":"<hr>";
	
	$refContenido1 = "( ".$_POST['refResumen1'].", ".$_POST['refResumen2']." )"."<hr>";
...
<?php echo $posts['CodPost'] ?>" al parecer llamamos a $_GET['CodPost'] y tenemos por un lado un $_GET['CodPost'] y por otro un $_POST['CodPost'] en index.php
*/
if(!empty($_POST['comentario']))
{
	...
	header('location: index.php');
	/* TODO: el primer algumento de notificaciones::agregar(1,... implica que se ha comentado y enviamos ese valor "1" como un valor clave para indicar el envio de una notificacion,recordar que $_POST['CodPost']) es un imput de tipo hidden para que al momento de escribir y enviar este pequeno formulario desde publicacion.php se envie en segundo plano debido a <input type="hidden" name="CodPost" value="<?php echo $posts['CodPost'] ?>"> */
}
...
{
/*
El valor recibido por mg con 0 implica que un usuario no ha seleccionado el OK(me gusta), y el 2 implica que es ya se ha seleccionado Ok, y es evidente que no se debe registrar mas de 1 OK por publicacion
TODO: He sustituido "false" por 0, nos da exactamente el mismo resultado, porque los valores de la columna "accion" que son valores 0 o 1 procesados en notificaciones::agregar , registran la notificacion hasta que el usuario la vea
*/
...
if($_GET['mg']==0){
	...
	/* en la funcion agregar de clase mg es en donde se insertaran los me gusta efectuados en publicacion.php para luego en notificaciones::agregar registrar la notificacion, peri si ya se ha puesto me gusta, no pasaremos por este condicional mas sera por ==2. En mg::agregar se procesa SQL de "insert into mg(CodLike, CodPost, CodUsua) values(null, :CodPost, :CodUsua)"); */

	notificaciones::agregar(false, $_GET['CodPost'], $_SESSION['CodUsua']);
	
	header('location: index.php');
	/* 
	false, es un valor que da lo mismo poner 0 para el argumento que se recibira como parametro sobre $accion (0 implica que es necesario establecer una notificacion). En notificaciones::agregar function agregar($accion, $CodPost, $CodUsua) , se invoca a la consulta SQL con "insert into notificaciones(CodNot, accion, CodPost, CodUsua, visto) values(null, :accion, :CodPost, :CodUsua, 0)");
	 */
}
if($_GET['mg']==2){
