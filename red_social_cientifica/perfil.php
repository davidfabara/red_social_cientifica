...
if(isset($_GET['CodUsua']))
{
	...
	*/
	...
	/*
	 Si el usuario esta vacio, retornamos a index.php 
	*/
...
{
	/*
	 desde el mismo perfil.php en el codigo embedido entre html y php de la seccion inferior, con <a href="perfil.php?CodUsua=<?php echo $_GET['CodUsua']; ?>&&agregar=<?php echo $_GET['CodUsua']; ?>">Agregar</a>  es en donde se envia al mismo perfil.php la variable $_GET['agregar'], un ususario de SESSION  que visite el perfil de un X usuario, tiene la posibilidad de enviar esta solicitud de amistad
	*/
...
{
	/*Primero se controla que el comentario no este vacio para luego ya sea las notificaciones o los me gusta para agregrarlos a la base de datos y luego notificarlos en la seccion de cabecera(superior) */
...
if(isset($_GET['mg']))
{
    ...
    El valor de accion puede ser 1 o 0 , 1 para comentarios y 0 para me gusta, un dato que 0=false,  de esta forma se procesa la accion de 0 en la base de datos 
    */
	if($_GET['mg']==0){
	...
	/*
	 OJO, aqui hay un bug, a diferencia de su homologo en index.php que cumple las mismas caracteriscticas, aqui en perfil.php pero hay un error que en perfil.php de no ubicar el header('location: index.php'); despues del todo, nos produce el error, a diferencia en index.php en el cual al hacer click en una publicacion que ya hemos puesto me gusta es evidente que no deberia ser necesario recargar la pagina como si se soluciona en index.php 
	 */
}
...
<div id="perfil">
	<ul>
		<li><img src="<?php echo $usuario[0]['foto_perfil']; ?>" alt="<?php echo "El nombre del usuario es:".$usuario[0]['nombre'];?>" id="img"></li>
		<li>
			<h3><?php echo $usuario[0]['nombre']; ?></h3>
			<ul>
				<li><strong>Discapacidad Visual: </strong><span><?php echo $usuario[0]['discapacidad']; ?></span></li>
				<li><strong>Profesion: </strong><span><?php echo $usuario[0]['profesion']; ?></span></li>
				<li><strong>Pais: </strong> <span><?php echo $usuario[0]['pais']; ?></span></li>
