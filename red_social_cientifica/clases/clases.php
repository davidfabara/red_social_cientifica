...
class post{
	...
	function mostrarTodo($amigos)
	{
		...
		;
   		*/
		$con = conexion("root", "");
		$consulta = $con->prepare("select U.CodUsua, U.nombre, U.foto_perfil, P.CodPost, P.titulo, P.fecha, P.categoria, P.autor, P.contenido, P.img, P.url from usuarios U inner join post P on U.CodUsua = P.CodUsua where P.CodUsua in($amigos) ORDER BY P.CodPost DESC");
		$consulta->execute();
		$resultado = $consulta->fetchAll();
		return $resultado;
        ...
        "in" nos devolveria tosos los registros de los elementos de la consulta de $amigos = amigos::codigos_amigos($_SESSION['CodUsua']) que como sabemos implica a (" select group_concat(usua_enviador,',', usua_receptor) as amigos from amigos where (usua_enviador = :CodUsua or usua_receptor = :CodUsua) and status = 1 "); , lo que sucede en post::mostrarTodo($amigos); es que $amigos a mas de la consulta SQL efectuada con la seleccion de    select group_concat(usua_enviador,',', usua_receptor) as amigos nos filtra esos valores separados por ",", que justamente el "in" procesa distintas variables separadas por coma   ,
	...
	function mostrarTodo_busqueda($buscar){
		$con = conexion("root", "");
		$consulta = $con->prepare("select U.CodUsua, U.nombre, U.foto_perfil, P.CodPost, P.titulo, P.autor, P.fecha, P.categoria,P.contenido, P.img, P.url from usuarios U inner join post P on U.CodUsua = P.CodUsua where P.titulo like :buscar ORDER BY P.CodPost DESC ");
